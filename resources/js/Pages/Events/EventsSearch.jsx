...
export default function EventsSearch({ t, setOpenSearch, events, setEventsFiltered, tags, resetFields, data, setData }) {
	...
	const handleSearch = (e) => {
        ...
        if(
	        ...
	        eventsFilter = events.filter((event) => {
				...
				if(data.name && data.name.length > 0 && !event.name.toLowerCase().includes(data.name.toLowerCase())){
					return false;
				}

				if(data.description && data.description.length > 0 && !event.description.toLowerCase().includes(data.description.toLowerCase())){
					return false;
				}

                if(data.location && data.location.length > 0 && !event.location.toLowerCase().includes(data.location.toLowerCase())){
                    return false;
                }

				if(data.tag_id && parseInt(event.tag_id) !== parseInt(data.tag_id)){
					return false;
				}
